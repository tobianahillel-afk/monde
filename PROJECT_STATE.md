...
The durable review chain is now `REVIEW-0003` through `REVIEW-0019`. Corrective test evidence is `TEST-0004`, `TEST-0005`, `TEST-0006`, and `TEST-0007`; superseded `TEST-0001` remains historical/non-gating evidence.
...
- REVIEW-0019 on exact frozen head `13f6b315df8d3734e90b212748f730e0d921c33c` found two defects: WORK-0001 was not machine-parseable because the RUN-3 flow mapping lacked its closing brace, and PROJECT_STATE/TEST-0006 copied a truncated TEST-0005 result SHA.

## REVIEW-0019 provenance and correction

REVIEW-0019 is truthfully imported under v7:

- one-shot authorization `c916d505f190a27d36e5d8fd4e595e970969e269` existed before materialization;
- initial import `40050f6fda6787feb42769cb6fa66547d5a1f7fb` records the externally completed `CHANGES_REQUIRED` review;
- review binding is `fd7459fd12ab245f37072a9126777f98a066fc0b`;
- authorization consumption is `4e7e103b7b4dc6567b4bf7f80164e97646bb3e5f` and binds `consumed_by_commit` to the exact import `40050f6fda6787feb42769cb6fa66547d5a1f7fb`.

REVIEW-0019/F-1 is corrected structurally in WORK-0001: the malformed RUN-3 flow mapping now has its required closing brace. REVIEW-0019/F-2 is corrected by using the exact resolvable TEST-0005 PASS result commit `ce962ec2ad199dd914ee4e4e8a66ec4e110fbfbe` everywhere that proof is copied.

Neither finding is considered resolved merely because these author corrections exist. A fresh exact-SHA TEST-0006 execution must revalidate the corrected machine-readable cold-resume state, followed by another independent L2.
...
- REQ-0020/0021/0022 are inspectable as the prematurely accepted atomic records and transitioned legally `ACCEPTED → SUPERSEDED` together in `9ca6b975e7abef9eadf86573f2c9e1cb07b89e3d`.
...
This separates two gates that are both required: a lifecycle-valid `PROPOSED → ACCEPTED` transition and the substantive Review Council/cold-read evidence required **before** that transition.
...
TEST-0007 remains the current generic v7 lifecycle proof on exact tree `669dcf7638746e07cc034cd69fa2cb08da133937`, result `87668f4e64453dca4c987252a8cb46e73793950c`, history pointer `87ec209eec614a0ff3be7a00a7565102042cad3f`. REVIEW-0019 did not modify generic lifecycle semantics.
...
That execution verifies atomic ownership, replacement/supersession traceability, proposal status, Review Council/cold-read sequencing and the prohibition on pre-review acceptance. It is evidence for independent review, not approval authority.

## TEST-0006 cold-resume proof routing

The TEST-0006 PASS that supported frozen head `13f6b315...` is no longer sufficient as current proof because REVIEW-0019 established that its tested tree contained malformed WORK-0001 YAML and copied an invalid TEST-0005 SHA. Historical executions remain preserved; the affected current proof must be rerun after both corrections.

**Transition-stable rule:** the canonical current execution of each TEST lives in its TEST record. At resume, inspect TEST-0006's current lifecycle state and latest valid exact tested tree. Continue legally from that state until a PASS exists that covers the corrected REVIEW-0019/WORK-0001/PROJECT_STATE semantics. Do not hardcode an intermediate READY/RUNNING step here and do not manufacture extra reruns after qualifying proof already exists.

Live PR #2 HEAD/check/thread truth remains external and is queried from GitHub rather than copied into canonical handover prose.
...
No finding is closed because the author produced a test PASS. REVIEW-0018/F-1 and REVIEW-0019/F-1/F-2 remain open pending a later fresh-context approval-capable L2 that reviews the proposed replacement requirements, verifies the machine-readability/SHA corrections and reviews the accumulated PR.

## Next action

1. Inspect TEST-0006's current lifecycle state and continue it legally until one exact-SHA PASS covers the corrected REVIEW-0019/WORK-0001/PROJECT_STATE handover, including machine-readable WORK-0001 and the exact TEST-0005 result SHA.
2. Once that qualifying PASS exists, reply to REVIEW-0019/F-1 and F-2 (and REVIEW-0018/F-1) with exact evidence, leave the threads unresolved, freeze the resulting PR #3 HEAD and request fresh-context L2 explicitly on that exact SHA.
3. The reviewer must inspect REQ-0023/0024/0025 while they remain PROPOSED, verify REVIEW-0018/F-1 and REVIEW-0019/F-1/F-2, and inspect the accumulated PR for any material P0/P1/P2 defect.
...
10. `registry/reviews/REVIEW-0003.yaml` through `REVIEW-0019.yaml`
