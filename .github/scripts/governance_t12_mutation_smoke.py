...
MUTATIONS = {
    ...
    "t12-terminal-import-binding": (
        "tools/governance/t11_closure.py",
        "            if not raw_import:\n",
        "            if False:\n",
    ),
    "t12-ambiguous-first-status": (
        "tools/governance/t11_closure.py",
        "            if len(boundaries) > 1:\n",
        "            if False:\n",
    ),
    "t12-executable-command-proof": (
        "tools/governance/t11_closure.py",
...
def main() -> int:
    ...
    for name, (target, needle, replacement) in MUTATIONS.items():
        ...
        if original.count(needle) != 1:
            print(f"T12 MUTATION ERROR {name}: target occurrence count != 1", file=sys.stderr)
            return 2
        with tempfile.TemporaryDirectory(prefix="monde-t12-mutation-") as temp_dir:
            temp = Path(temp_dir)
            ...
            if proc.returncode == 0:
                print(f"T12 MUTATION SURVIVED {name}", file=sys.stderr)
                continue
            print(f"T12 MUTATION KILLED {name}")
            killed += 1
    print(f"MONDE T12 mutation smoke: {killed}/{len(MUTATIONS)} critical mutations killed")
