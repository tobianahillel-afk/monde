...
REVIEW_OUTCOMES_APPROVING = {"APPROVE", "APPROVE_WITH_FOLLOWUP", "APPROVED", "APPROVED_WITH_NONBLOCKING_FOLLOWUPS", "PASS"}
INDEPENDENCE_RANK = {"L0": 0, "L1": 1, "L2": 2, "L3": 3}
...
def independence_rank(raw: Any) -> int:
    value = str(raw or "").upper()
    level = value.split("_", 1)[0]
    return INDEPENDENCE_RANK.get(level, -1)
...
class Validator:
        ...
        for kind, prefix in PREFIX_BY_KIND.items():
            ...
            for path in sorted(d.glob("*.yaml")):
                ...
                if not isinstance(data, dict):
                    ...
                    continue
                r = Record(kind, path, data)
                self.records.append(r)
                if not r.id:
                    self.add(path, "ID_REQUIRED", "registry record is missing id")
                elif not r.id.startswith(prefix):
                    self.add(path, "ID_PREFIX", f"id {r.id!r} must start with {prefix}")
                elif r.id in self.by_id:
                    self.add(path, "ID_UNIQUE", f"duplicate id {r.id}")
                else:
                    self.by_id[r.id] = r
    ...
    def validate_schemas(self) -> None:
        for r in self.records:
            ...
            if not path.exists():
                self.add(r.path, "SCHEMA_MISSING", f"missing schema {path.relative_to(self.root)}")
                continue
            try:
                schema = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError) as exc:
                self.add(path, "SCHEMA_PARSE", str(exc))
                continue
            for error in Draft202012Validator(schema).iter_errors(r.data):
    ...
    def iter_strings(self, value: Any) -> Iterable[str]:
        if isinstance(value, str):
            ...
            for x in value.values():
                yield from self.iter_strings(x)
        ...
        for r in self.records:
            for text in self.iter_strings(r.data):
                for match in ID_PATTERN.finditer(text):
                    ref = match.group(0)
                    if ref != r.id and ref not in self.by_id:
                        self.add(r.path, "REFERENCE", f"unknown registry reference {ref}")
        ...
        for r in self.records:
            for key in ("read_before", "affected_docs", "affected_schemas", "affected_paths"):
                ...
                if not isinstance(vals, list):
                    self.add(r.path, "PATH_LIST", f"{key} must be a list")
                    continue
                for val in vals:
                    if not isinstance(val, str):
                        self.add(r.path, "PATH_TYPE", f"{key} contains non-string path {val!r}")
                    elif not (self.root / val).exists():
                        self.add(r.path, "PATH_EXISTS", f"{key} path does not exist: {val}")

    def validate_work_tasks(self, r: Record) -> None:
        plan = r.data.get("implementation_plan", {}) or {}
        tasks = plan.get("tasks", []) or []
        runs = plan.get("planned_runs", []) or []
        ids = {t.get("id") for t in tasks if isinstance(t, dict) and t.get("id")}
        for t in tasks:
            if not isinstance(t, dict):
                self.add(r.path, "TASK_SHAPE", "task must be a mapping")
                continue
            for dep in t.get("depends_on", []) or []:
                if dep not in ids:
                    self.add(r.path, "TASK_DEP", f"task {t.get('id')} depends on unknown task {dep}")
        for run in runs:
            if not isinstance(run, dict):
                self.add(r.path, "RUN_SHAPE", "planned run must be a mapping")
                continue
            for tid in run.get("tasks", []) or []:
                if tid not in ids:
                    self.add(r.path, "RUN_TASK", f"run {run.get('id')} references unknown task {tid}")

    def validate_review_evidence(self, r: Record) -> None:
        plan = r.data.get("review_plan", {}) or {}
        required = set(plan.get("required_hats", []) or [])
        target = str(plan.get("independence_level", "") or "")
        target_rank = independence_rank(target)
        completed = plan.get("completed_reviews", []) or []
        if required and not completed:
            self.add(r.path, "DONE_REVIEW", "DONE work item requires completed review evidence")
            return
        ...
        for rid in completed:
            ...
            for finding in data.get("findings", []) or []:
                if (
                    ...
                    and finding.get("severity") in {"R0_CRITICAL", "R1_BLOCKER", "R2_MAJOR"}
                    and finding.get("disposition") not in {"RESOLVED", "ACCEPTED"}
                ):
                    self.add(r.path, "DONE_REVIEW", f"review {rid} retains blocking finding {finding.get('id')}")

        if not required.issubset(covered):
            self.add(r.path, "DONE_REVIEW", f"completed reviews miss required hats {sorted(required - covered)}")
        if target_rank >= 0 and max_rank < target_rank:
            self.add(r.path, "DONE_REVIEW", f"no completed review satisfies independence {target}")

    def validate_done_work(self, r: Record) -> None:
        crit = r.data.get("acceptance_criteria", []) or []
        if not crit:
            self.add(r.path, "DONE_AC", "DONE work item must have acceptance criteria")
        for item in crit:
            if not isinstance(item, dict) or item.get("status") != "DONE":
                self.add(r.path, "DONE_AC", f"incomplete acceptance criterion {item!r}")
        ...
        for key in (
            ...
            if comp.get(key) is not True:
                self.add(r.path, "DONE_COMPLETION", f"DONE work item requires completion.{key}=true")
        self.validate_review_evidence(r)

    def validate_work_graph(self) -> None:
        works = {k: v for k, v in self.by_id.items() if v.kind == "work-items"}
        edges: dict[str, list[str]] = {}
        for wid, r in works.items():
            deps = r.data.get("depends_on", []) or []
            edges[wid] = []
            if not isinstance(deps, list):
                self.add(r.path, "WORK_DEPS", "depends_on must be a list")
                continue
            for dep in deps:
                if dep not in works:
                    self.add(r.path, "WORK_DEP_EXISTS", f"unknown work dependency {dep}")
                else:
                    edges[wid].append(dep)
            self.validate_work_tasks(r)
            if r.data.get("status") == "DONE":
                self.validate_done_work(r)

        state: dict[str, int] = {}

        def visit(node: str, stack: list[str]) -> None:
            if state.get(node) == 1:
                self.add(works[node].path, "WORK_CYCLE", f"dependency cycle detected: {' -> '.join(stack + [node])}")
                return
            if state.get(node) == 2:
                return
            state[node] = 1
    ...
    def check_due(self, r: Record, raw: Any, rule: str) -> None:
        if raw in (None, ""):
            return
        try:
            due = date.fromisoformat(str(raw))
        except ValueError:
            self.add(r.path, rule, f"invalid ISO date {raw!r}")
            return
        if due < self.today:
            self.add(r.path, rule, f"review/validation date {due} is overdue")

    def validate_due_dates(self) -> None:
        for r in self.records:
            if r.kind == "assumptions" and r.data.get("status") == "OPEN":
                self.check_due(r, (r.data.get("validation") or {}).get("target_date"), "ASSUMPTION_DUE")
            if r.kind == "risks" and r.data.get("status") == "OPEN":
                self.check_due(r, r.data.get("review_by"), "RISK_DUE")
    ...
    def validate_markdown(self) -> None:
        for path in sorted(self.root.rglob("*.md")):
            ...
            try:
                ...
                continue
            if "Canonical: Yes" in text and "Status: Accepted" in text and PLACEHOLDER.search(text):
            ...
            for raw in MARKDOWN_LINK.findall(text):
                ...
                if not (path.parent / clean).resolve().exists():
                    self.add(path, "MARKDOWN_LINK", f"broken relative link: {raw}")
    ...
    def validate_progress(self) -> None:
        ...
        for phase in (data.get("phases") or {}).values():
            for lot in (phase.get("lots") or {}).values():
                for sub in (lot.get("sublots") or {}).values():
                    for wid, state in (sub.get("work_items") or {}).items():
                        found[wid] = state
        works = {r.id: r for r in self.records if r.kind == "work-items"}
        for wid in works:
            if wid not in found:
                self.add(path, "PROGRESS_WORK", f"work item {wid} is missing from progress matrix")
        for wid, state in found.items():
            ...
            if status != r.data.get("status"):
                self.add(path, "PROGRESS_STATUS", f"{wid} matrix status {status!r} != work item status {r.data.get('status')!r}")
    ...
    def validate_action_pins(self) -> None:
        workflow_dir = self.root / ".github/workflows"
        paths = sorted(workflow_dir.glob("*.yml")) if workflow_dir.exists() else []
        for path in paths:
            try:
                        ...
                        if key == "uses" and isinstance(item, str):
                            if item.startswith("./"):
                                ...
                                if not PINNED_DOCKER.fullmatch(item):
                                    self.add(path, "ACTION_PIN", f"docker action must use sha256 digest: {item}")
                            elif not PINNED_ACTION.fullmatch(item):
                                self.add(path, "ACTION_PIN", f"action must use full commit SHA: {item}")
                        walk(item)
                elif isinstance(value, list):
                    for item in value:
                        walk(item)
    ...
    def validate_secrets(self) -> None:
        ...
        for path in self.root.rglob("*"):
            if (
                not path.is_file()
                or any(part in path.parts for part in skip)
                or path.suffix.lower() in {".png", ".jpg", ".jpeg", ".zip", ".pdf"}
            ):
                continue
            try:
                text = path.read_text(encoding="utf-8")
            ...
            for pattern in SECRET_PATTERNS:
                if pattern.search(text):
                    self.add(path, "SECRET_PATTERN", "high-confidence secret/private-key pattern detected")
                    break
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    validator = Validator(Path(args.root), today)
    issues = validator.run()
    for issue in issues:
        print(issue.render(), file=sys.stderr if issue.severity == "ERROR" else sys.stdout)
    if args.json_out:
        Path(args.json_out).write_text(json.dumps([asdict(x) for x in issues], indent=2), encoding="utf-8")
    errors = sum(issue.severity == "ERROR" for issue in issues)
    print(f"MONDE governance validation: {errors} error(s), {len(issues) - errors} warning(s), {len(validator.records)} record(s)")
