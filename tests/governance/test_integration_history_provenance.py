...
def init_history(root: Path) -> tuple[str, str, str, str, str]:
    git(root, "init")
...
"""
    ...
    return base, malformed, malformed_blob, repaired, repaired_blob
...
def test_exact_malformed_history_does_not_become_delete_and_reintroduction(tmp_path: Path) -> None:
    base, malformed, malformed_blob, repaired, repaired_blob = init_history(tmp_path)
    ...
    assert c.historical_malformed_yaml_allowed(tmp_path, path, malformed, head, repaired_sha=repaired)
...
def test_malformed_history_exception_fails_closed_on_structure_flags_and_repair(tmp_path: Path) -> None:
    _, malformed, malformed_blob, repaired, repaired_blob = init_history(tmp_path)
    path = "registry/work-items/WORK-1.yaml"
    ...
    for index, value in enumerate(variants):
        ...
        assert not c.historical_malformed_yaml_allowed(tmp_path, path, malformed, variant_head)

    write(tmp_path, "registry/integration-provenance.yaml", provenance(path, malformed, malformed_blob, repaired, repaired_blob))
    good_head = commit(tmp_path, "restore exact exception")
    assert not c.historical_malformed_yaml_allowed(tmp_path, path, malformed, good_head, repaired_sha="f" * 40)
...
def test_malformed_history_requires_exact_blob_and_ancestry(tmp_path: Path) -> None:
    ...
    assert not c.historical_malformed_yaml_allowed(tmp_path, path, malformed, head)
    ...
    assert not c.historical_malformed_yaml_allowed(tmp_path, path, malformed, current)
