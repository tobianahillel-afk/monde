...
def write(root,path,data):p=root/path;p.parent.mkdir(parents=True,exist_ok=True);p.write_text(yaml.safe_dump(data) if isinstance(data,dict) else data)
...
def repo(tmp_path):
    ...
    w={'id':'WORK-1','status':'IN_PROGRESS','purpose':'p','scope':{'in':['a'],'out':[]},'acceptance_criteria':[{'id':'AC','description':'x'}],'assurance':{'level':'A3'},'review_plan':{'completed_reviews':[]}}
    write(tmp_path,'registry/work-items/WORK-1.yaml',w);write(tmp_path,'x.txt','a');return commit(tmp_path,'base')
